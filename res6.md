...
\begin{itemize}
  \item {\rightskip=0pt B.S. in Computer Science from Florida International University\hfill Expected Apr. 2027 \,\textbar\, GPA 3.76\par}
  \item {\rightskip=0pt A.A. in Computer Science from Miami Dade College\hfill Graduated Dec. 2024 \,\textbar\, GPA 3.81\par}
\end{itemize}
...
\begin{itemize}
  ...
  \item Created component tests for the dashboards and inventory pages with Vitest and React Testing Library, end-to-end app-flow tests with Playwright, and integration tests confirming the frontend worked with NGC's RESTful API.
  \item Built a reusable project template with CI/CD built in so any team could launch a new NGC app quickly.
\end{itemize}

\entry{Software Engineer}{ at CodeCrunch, Miami, FL}{Sept. 2025 to Apr. 2026}
\begin{itemize}
  \item Redesigned an education platform's website, migrating a HTML, CSS, and JavaScript codebase to Next.js and merging 8 projects into one so it was easier to maintain and scale.
  ...
  \item Created AI tools powered by OpenAI models that automated research and marketing so non-technical users could use them anytime.
...
\begin{itemize}
  \item Built Bash, Terraform, and Flox automation that set up each app's environment automatically, so engineers could run any of the 14+ apps locally with one command instead of by hand.
...
\entry{Software Engineer}{ at CodePath, Miami, FL}{May 2024 to Apr. 2025}
\begin{itemize}
  \item Led a team of 3 to build Errorly, a React, Node.js, and Railway app where developers get help from other developers, managing Git issues, branches, and code reviews so the team merged code safely.
...
\entry{Software Engineer}{ at INIT, Miami, FL}{Oct. 2024 to Dec. 2024}
\begin{itemize}
  \item Built sign-up and login with Google OAuth and email-code verification using FastAPI and a Supabase (Postgres) SQL database so users could create accounts and log in safely.
...
\begin{itemize}
  \item Built a Python and Flask server on an NVIDIA Jetson Orin Nano running Ubuntu Linux that connected a Next.js app to an Arduino and used computer vision to track cards, so the robot shuffled and dealt accurately on its own.
...
\begin{itemize}
  ...
  \item Developed it with an MVC structure and role-based access control (RBAC) so each user's tasks stayed private from each other, and admins could manage the app securely.
\end{itemize}
