#!/usr/bin/env node
/**
 * Generate per-sector instruction files consumed by Codex sector workers.
 */
...
const OUT_DIR = path.join(ROOT, "job/swarm/sectors");
...
function buildInstruction(sector) {
  ...
  return `# Codex Sector Worker — ${sector.label}

**Assignment**
- Sector key: \`${sector.key}\`
- Working file: \`${sector.file}\`
- Companies owned by this sector: ${sector.count}
- You may edit **only** \`${sector.file}\`.
...
- Check \`job/swarm/codex-sector-stop.json\` before starting each new company search and again before writing JSON. If it exists and either has \`"all": true\` or includes \`${sector.key}\` in \`"sectors"\`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
\`\`\`bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("${sector.file}","utf8"));
...
console.log("=== SECTOR ${sector.key} STATUS ===");
console.log("File: ${sector.file}");
...
Use a small Node script or safe JSON-aware edit against \`${sector.file}\` only. Preserve JSON formatting with two-space indentation.
...
function main() {
  ...
  for (const sector of manifest.sectors) {
    const outPath = path.join(OUT_DIR, `${sector.key}.md`);
    fs.writeFileSync(outPath, buildInstruction(sector), "utf8");
