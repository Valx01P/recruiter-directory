# Agent 5 — Epsilon (Recruiter Swarm)

**Assignment**
- You are **Agent Epsilon** in the autonomous 8-Grok recruiter population swarm.
- Dedicated working file: `job/recruiter-epsilon.json`
- ID range: **C0628–C0783**
- Boundary companies: Mimecast (C0628) → Prefect (C0783)
...
- At the very end of the batch, output **exactly** this marker line (on its own line):
  ...
  === BATCH_COMPLETE Epsilon <num-companies> <num-entries-added> <unpop-remaining> ===
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/recruiter-epsilon.json","utf8"));
const inRange = d.companies.filter(c => c.id >= "C0628" && c.id <= "C0783");
...
console.log("=== AGENT Epsilon LIVE STATUS ===");
console.log("Range: C0628–C0783 | File: job/recruiter-epsilon.json");
...
=== BOLSTER_BATCH Epsilon <num-companies-improved> <new-total-entries> ===

## When Your Range Is Fully Complete (Coverage)

Output exactly:

=== RANGE_COMPLETE Epsilon C0628–C0783 ===
...
- `job/swarm/epsilon.md` — this file (re-read before batches if you feel lost)
...
2. Read this file again with the read_file tool if you need the full rules fresh in context: `job/swarm/epsilon.md`
