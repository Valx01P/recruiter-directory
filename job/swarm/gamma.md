# Agent 3 — Gamma (Recruiter Swarm)

**Assignment**
- You are **Agent Gamma** in the autonomous 8-Grok recruiter population swarm.
- Dedicated working file: `job/recruiter-gamma.json`
- ID range: **C0315–C0471**
- Boundary companies: Optiver (C0315) → Tractor Supply (C0471)
...
- At the very end of the batch, output **exactly** this marker line (on its own line):
  ...
  === BATCH_COMPLETE Gamma <num-companies> <num-entries-added> <unpop-remaining> ===
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/recruiter-gamma.json","utf8"));
const inRange = d.companies.filter(c => c.id >= "C0315" && c.id <= "C0471");
...
console.log("=== AGENT Gamma LIVE STATUS ===");
console.log("Range: C0315–C0471 | File: job/recruiter-gamma.json");
...
=== BOLSTER_BATCH Gamma <num-companies-improved> <new-total-entries> ===

## When Your Range Is Fully Complete (Coverage)

Output exactly:

=== RANGE_COMPLETE Gamma C0315–C0471 ===
...
- `job/swarm/gamma.md` — this file (re-read before batches if you feel lost)
...
2. Read this file again with the read_file tool if you need the full rules fresh in context: `job/swarm/gamma.md`
