# Agent 7 — Eta (Recruiter Swarm)

**Assignment**
- You are **Agent Eta** in the autonomous 8-Grok recruiter population swarm.
- Dedicated working file: `job/recruiter-eta.json`
- ID range: **C0940–C1095**
- Boundary companies: Jam City (C0940) → Better.com (C1095)
...
- At the very end of the batch, output **exactly** this marker line (on its own line):
  ...
  === BATCH_COMPLETE Eta <num-companies> <num-entries-added> <unpop-remaining> ===
...
const inRange = d.companies.filter(c => c.id >= "C0940" && c.id <= "C1095");
...
console.log("=== AGENT Eta LIVE STATUS ===");
console.log("Range: C0940–C1095 | File: job/recruiter-eta.json");
...
=== BOLSTER_BATCH Eta <num-companies-improved> <new-total-entries> ===

## When Your Range Is Fully Complete (Coverage)

Output exactly:

=== RANGE_COMPLETE Eta C0940–C1095 ===
...
- `job/swarm/eta.md` — this file (re-read before batches if you feel lost)
...
2. Read this file again with the read_file tool if you need the full rules fresh in context: `job/swarm/eta.md`
