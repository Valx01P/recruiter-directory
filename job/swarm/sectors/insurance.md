# Codex Sector Worker — Insurance

**Assignment**
- Sector key: `insurance`
- Working file: `job/sectors/recruiter-insurance.json`
- Companies owned by this sector: 137
- You may edit **only** `job/sectors/recruiter-insurance.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `insurance` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-insurance.json","utf8"));
...
console.log("=== SECTOR insurance STATUS ===");
console.log("File: job/sectors/recruiter-insurance.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-insurance.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE insurance <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
