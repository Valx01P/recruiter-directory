# Codex Sector Worker — PropTech / Real Estate

**Assignment**
- Sector key: `proptech`
- Working file: `job/sectors/recruiter-proptech.json`
- Companies owned by this sector: 90
- You may edit **only** `job/sectors/recruiter-proptech.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `proptech` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-proptech.json","utf8"));
...
console.log("=== SECTOR proptech STATUS ===");
console.log("File: job/sectors/recruiter-proptech.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-proptech.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE proptech <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
