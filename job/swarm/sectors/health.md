# Codex Sector Worker — Healthcare / Bio

**Assignment**
- Sector key: `health`
- Working file: `job/sectors/recruiter-health.json`
- Companies owned by this sector: 102
- You may edit **only** `job/sectors/recruiter-health.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `health` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-health.json","utf8"));
...
console.log("=== SECTOR health STATUS ===");
console.log("File: job/sectors/recruiter-health.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-health.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE health <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
