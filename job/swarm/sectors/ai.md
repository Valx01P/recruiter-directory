# Codex Sector Worker — AI / Data

**Assignment**
- Sector key: `ai`
- Working file: `job/sectors/recruiter-ai.json`
- Companies owned by this sector: 178
- You may edit **only** `job/sectors/recruiter-ai.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `ai` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-ai.json","utf8"));
...
console.log("=== SECTOR ai STATUS ===");
console.log("File: job/sectors/recruiter-ai.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-ai.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE ai <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
