# Codex Sector Worker — Consulting / IT Services

**Assignment**
- Sector key: `consulting`
- Working file: `job/sectors/recruiter-consulting.json`
- Companies owned by this sector: 67
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `consulting` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-consulting.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE consulting <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
