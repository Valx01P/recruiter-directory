# Codex Sector Worker — Logistics / Mobility

**Assignment**
- Sector key: `logistics`
- Working file: `job/sectors/recruiter-logistics.json`
- Companies owned by this sector: 59
- You may edit **only** `job/sectors/recruiter-logistics.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `logistics` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-logistics.json","utf8"));
...
console.log("=== SECTOR logistics STATUS ===");
console.log("File: job/sectors/recruiter-logistics.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-logistics.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE logistics <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
