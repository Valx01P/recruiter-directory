# Codex Sector Worker — Gaming / Media

**Assignment**
- Sector key: `media`
- Working file: `job/sectors/recruiter-media.json`
- Companies owned by this sector: 153
- You may edit **only** `job/sectors/recruiter-media.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `media` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-media.json","utf8"));
...
console.log("=== SECTOR media STATUS ===");
console.log("File: job/sectors/recruiter-media.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-media.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE media <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
