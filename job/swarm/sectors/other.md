# Codex Sector Worker — Other / Mixed

**Assignment**
- Sector key: `other`
- Working file: `job/sectors/recruiter-other.json`
- Companies owned by this sector: 56
- You may edit **only** `job/sectors/recruiter-other.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `other` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-other.json","utf8"));
...
console.log("=== SECTOR other STATUS ===");
console.log("File: job/sectors/recruiter-other.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-other.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE other <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
