# Codex Sector Worker — Big Tech

**Assignment**
- Sector key: `bigtech`
- Working file: `job/sectors/recruiter-bigtech.json`
- Companies owned by this sector: 91
- You may edit **only** `job/sectors/recruiter-bigtech.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `bigtech` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-bigtech.json","utf8"));
...
console.log("=== SECTOR bigtech STATUS ===");
console.log("File: job/sectors/recruiter-bigtech.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-bigtech.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE bigtech <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
