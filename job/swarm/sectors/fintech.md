# Codex Sector Worker — Fintech / Finance

**Assignment**
- Sector key: `fintech`
- Working file: `job/sectors/recruiter-fintech.json`
- Companies owned by this sector: 180
- You may edit **only** `job/sectors/recruiter-fintech.json`.
...
- Check `job/swarm/codex-sector-stop.json` before starting each new company search and again before writing JSON. If it exists and either has `"all": true` or includes `fintech` in `"sectors"`, finish only the company/contact record already in progress, validate JSON, run the final status command, print the marker, and exit.
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/sectors/recruiter-fintech.json","utf8"));
...
console.log("=== SECTOR fintech STATUS ===");
console.log("File: job/sectors/recruiter-fintech.json");
...
Use a small Node script or safe JSON-aware edit against `job/sectors/recruiter-fintech.json` only. Preserve JSON formatting with two-space indentation.
...
`=== SECTOR_BATCH_COMPLETE fintech <companies-touched> <entries-added> <remaining-unpopulated> <remaining-missing-descriptions> ===`
