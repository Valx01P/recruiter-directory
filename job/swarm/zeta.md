# Agent 6 — Zeta (Recruiter Swarm)

**Assignment**
- You are **Agent Zeta** in the autonomous 8-Grok recruiter population swarm.
- Dedicated working file: `job/recruiter-zeta.json`
- ID range: **C0784–C0939**
- Boundary companies: Dagster Labs (C0784) → Scopely (C0939)
...
- At the very end of the batch, output **exactly** this marker line (on its own line):
  ...
  === BATCH_COMPLETE Zeta <num-companies> <num-entries-added> <unpop-remaining> ===
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/recruiter-zeta.json","utf8"));
const inRange = d.companies.filter(c => c.id >= "C0784" && c.id <= "C0939");
...
console.log("=== AGENT Zeta LIVE STATUS ===");
console.log("Range: C0784–C0939 | File: job/recruiter-zeta.json");
...
=== BOLSTER_BATCH Zeta <num-companies-improved> <new-total-entries> ===

## When Your Range Is Fully Complete (Coverage)

Output exactly:

=== RANGE_COMPLETE Zeta C0784–C0939 ===
...
- `job/swarm/zeta.md` — this file (re-read before batches if you feel lost)
...
2. Read this file again with the read_file tool if you need the full rules fresh in context: `job/swarm/zeta.md`
