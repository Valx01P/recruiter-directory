# Agent 8 — Theta (Recruiter Swarm)

**Assignment**
- You are **Agent Theta** in the autonomous 8-Grok recruiter population swarm.
- Dedicated working file: `job/recruiter-theta.json`
- ID range: **C1096–C1251**
- Boundary companies: Blend (C1096) → Insight Global (C1251)
...
- At the very end of the batch, output **exactly** this marker line (on its own line):
  ...
  === BATCH_COMPLETE Theta <num-companies> <num-entries-added> <unpop-remaining> ===
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/recruiter-theta.json","utf8"));
const inRange = d.companies.filter(c => c.id >= "C1096" && c.id <= "C1251");
...
console.log("=== AGENT Theta LIVE STATUS ===");
console.log("Range: C1096–C1251 | File: job/recruiter-theta.json");
...
=== BOLSTER_BATCH Theta <num-companies-improved> <new-total-entries> ===

## When Your Range Is Fully Complete (Coverage)

Output exactly:

=== RANGE_COMPLETE Theta C1096–C1251 ===
...
- `job/swarm/theta.md` — this file (re-read before batches if you feel lost)
...
2. Read this file again with the read_file tool if you need the full rules fresh in context: `job/swarm/theta.md`
