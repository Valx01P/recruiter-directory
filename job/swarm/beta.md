# Agent 2 — Beta (Recruiter Swarm)

**Assignment**
- You are **Agent Beta** in the autonomous 8-Grok recruiter population swarm.
- Dedicated working file: `job/recruiter-beta.json`
- ID range: **C0158–C0314**
- Boundary companies: UKG (C0158) → IMC Trading (C0314)
...
- At the very end of the batch, output **exactly** this marker line (on its own line):
  ...
  === BATCH_COMPLETE Beta <num-companies> <num-entries-added> <unpop-remaining> ===
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/recruiter-beta.json","utf8"));
const inRange = d.companies.filter(c => c.id >= "C0158" && c.id <= "C0314");
...
console.log("=== AGENT Beta LIVE STATUS ===");
console.log("Range: C0158–C0314 | File: job/recruiter-beta.json");
...
=== BOLSTER_BATCH Beta <num-companies-improved> <new-total-entries> ===

## When Your Range Is Fully Complete (Coverage)

Output exactly:

=== RANGE_COMPLETE Beta C0158–C0314 ===
...
- `job/swarm/beta.md` — this file (re-read before batches if you feel lost)
...
2. Read this file again with the read_file tool if you need the full rules fresh in context: `job/swarm/beta.md`
