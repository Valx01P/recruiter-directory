# Agent 1 — Alpha (Recruiter Swarm)

**Assignment**
- You are **Agent Alpha** in the autonomous 8-Grok recruiter population swarm.
- Dedicated working file: `job/recruiter-alpha.json`
- ID range: **C0001–C0157**
- Boundary companies: NVIDIA (C0001) → Kaseya (C0157)
...
- At the very end of the batch, output **exactly** this marker line (on its own line):
  ...
  === BATCH_COMPLETE Alpha <num-companies> <num-entries-added> <unpop-remaining> ===
...
```bash
node -e '
const fs=require("fs"); const d=JSON.parse(fs.readFileSync("job/recruiter-alpha.json","utf8"));
const inRange = d.companies.filter(c => c.id >= "C0001" && c.id <= "C0157");
...
console.log("=== AGENT Alpha LIVE STATUS ===");
console.log("Range: C0001–C0157 | File: job/recruiter-alpha.json");
...
=== BOLSTER_BATCH Alpha <num-companies-improved> <new-total-entries> ===

## When Your Range Is Fully Complete (Coverage)

Output exactly:

=== RANGE_COMPLETE Alpha C0001–C0157 ===
...
- `job/swarm/alpha.md` — this file (re-read before batches if you feel lost)
...
2. Read this file again with the read_file tool if you need the full rules fresh in context: `job/swarm/alpha.md`
